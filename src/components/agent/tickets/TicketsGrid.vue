<template>
  <div>
    <div class="row">
      <h2>All Tickets</h2>
      <a href="http://localhost:3000/pdf/closed_tickets_last_month">last month's closed tickets</a>
    </div>
...
export default {
  ...
  methods: {
    fetchData () {
      this.$http.get('http://localhost:3000/agent/tickets', { headers: { 'X-Api-Key': this.readCookie('token') } }).then((response) => {
